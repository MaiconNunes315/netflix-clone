
import {Bell } from "phosphor-react"
import {BsFillCaretDownFill, GoSearch,IoCloseSharp} from "react-icons/all"
import { useState,useEffect } from "react"
import logo from "../../assets/logo.png"
import "./header-nav.scss"


export default function Header() {

    const [showInput, setShowInput] = useState(false);
    const [valueSearch, setValueSearch] = useState("");
    const [widthScreen, setWidthScreen] = useState()
    const [activeClass, setActiveClass] = useState(false)

 
    
    
    const navigate = ["Início","Séries","Filmes","Bombando","Minha lista","Navegar por idioma"]
    
    

    return (
      
      <header className="header" >
          
          <nav>
                <img className="nav" src={logo} alt='netflix' />
                
                
                    {
                    document.body.offsetWidth < 1012 ? (
                        
                        <div className="navigate">

                        <BsFillCaretDownFill  onMouseMove={()=>{setActiveClass(true)}} onMouseLeave={()=>setActiveClass(false)} className="down" />
                    
                    <ul className={activeClass ?(""):("hidden")}>
                                <hr className="hr" />
                                
                {navigate.map((list, index) => (
                    <li key={index}>{ list}</li>                     
                ))}
                                
                    </ul> 
                        </div>
                            
                            
                    ) : (
                        <div className="list">
                        
                        <ul>
                        
                    {navigate.map((list, index) => (
                        <li key={index}>{ list}</li>                     
                    ))}
                        </ul> 
                            </div>)
                        
                }
                        
              
              
          </nav>

          <div className="header-right">
              
                <div className="search-box">

                    <label htmlFor="search" onClick={()=>{ setShowInput(showInput === false ? true : false)}}>
                        <GoSearch className={`search-icon ${showInput ? "left" : "right"} `} />
                        <IoCloseSharp onClick={() => { setValueSearch(""); setShowInput(false)}} className={`close-icon right ${(valueSearch.length > 1) ? "block":"hidden" }`}/>
                     </label>
                    
                    <input onChange={(e) => {

                        setValueSearch(e.target.value)
                        
                    }}  value={valueSearch} id="search" placeholder="Títulos, gente e gênero" className={`${(showInput) ? "block" : "hidden"} ${(valueSearch.length > 1 ? "block" : null)}`} type="text" />
              
                </div>
              
              <span className="kids">Infantil</span>

              <div className="div-bell">
                  <Bell size={30} color="#a52c2c" weight="light" />
                  <div className="bell-notification">1</div>
              </div>

                <div className="users">
                    
                        <img className="img-users" src="data:image/png;base64," alt="usuário" />
                        <BsFillCaretDownFill className="down"/>
                    
                </div>                 
              
          </div>
    </header>
  )
}
