import { MagnifyingGlass,Bell } from "phosphor-react"
import { useState } from "react"
import logo from "../../assets/logo.png"


import Nav from "./Nav"
export default function Header() {

    const [showInput, setShowInput] = useState(false)

  return (
      <header className='flex justify-between px-16 py-3'>
          
          <nav className="flex">
          <img className="w-28 mr-12 h-9 my-auto cursor-pointer" src={logo} alt='netflix' />
              <Nav list="Início" />
              <Nav list="Séries" />
              <Nav list="Filmes" />
              <Nav list="Bombando" />
              <Nav list="Minha lista" />
              <Nav list="Navegar por idiomas" />
          </nav>
          
          

          <div className="flex items-center justify-between font-medium gap-5">
              
              
              <label htmlFor="search" onClick={()=>{ setShowInput(showInput === false ? true : false)}}>
                  <MagnifyingGlass  size={30} color="#a52c2c" weight="light" />
              </label>
              <input id="search" className={`${showInput?"flex":"hidden"} bg-transparent`} type="text" />
              

              <span>Infantil</span>

              <div className="relative">
                  <Bell size={30} color="#a52c2c" weight="light" />
                  <div className="rounded-full bg-red-600 w-4 h-4 text-xs text-center absolute top-0 right-0">1</div>
              </div>

              <div >
                  
                 <img className="w-10 rounded-full" src="data:image/png;base64," alt="usuário"/>
              </div>
          </div>
    </header>
  )
}
